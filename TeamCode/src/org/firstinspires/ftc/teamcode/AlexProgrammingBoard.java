...
@TeleOp(name = "Alex Programming Demo", group = "Test")
public class AlexProgrammingBoard extends OpMode {

    AlexMotor m = new AlexMotor();
    AlexServo s = new AlexServo();
    ...
    public void init(){
        ...
        m.init(hardwareMap);
        m.moveToPosition(1082,1.0);
        s.init(hardwareMap);

     //   s = hardwareMap.get(AlexServo.class, "servo");
        s.setPosition(0.5);
       ...
       MotorConfigurationType motorConfigType = m.getMotorType();
    ...
    public void loop(){
if(test==0&&!digitalChannel.getState()) {
    m.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
    test=1;
}

        //m.setPower(analogInput.getVoltage()/5);

//        m.setPower(-gamepad1.left_stick_y);
        //s.setPosition(0.5*(gamepad1.right_stick_y + 1));
